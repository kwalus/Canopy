...
- **Antecedent deck retry cleanup (`0.4.151`-`0.4.153`)** — The repost/variant deck-open path was trimmed back to the intended **direct row open + compositor pass + short `requestAnimationFrame` retry** flow. Repost/variant **Deck** buttons now prefer opening the antecedent deck **in place** from the current thread/feed view, with the **`open_deck=1`** deep-link only as fallback when the source row is not already loaded.
