...
def _serialize_channel_message_for_response(
    message: Any,
    *,
    viewer_id: str,
    db_manager: Any,
...
def create_api_blueprint() -> Blueprint:
    ...
    def update_feed_post(post_id):
        ...
        try:
            ...
            if success:
                ...
                # Sync inline collaboration cards from edited content
                try:
                    collab_card_manager = current_app.config.get('COLLAB_CARD_MANAGER')
                    if collab_card_manager:
                        effective_visibility = visibility_enum.value if visibility_enum else post.visibility.value
                    ...
                    logger.warning(f"Inline collaboration card sync failed on feed edit: {card_err}")
    ...
    def get_channel_messages(channel_id):
        ...
        try:
            ...
            out = []
            for message in messages:
                d = _serialize_channel_message_for_response(
                    message,
                    viewer_id=g.api_key_info.user_id,
                    db_manager=db_manager,
    ...
    def update_channel_message(channel_id, message_id):
        ...
        try:
            ...
            with db_manager.get_connection() as conn:
                row = conn.execute(
                    "SELECT user_id, content, created_at, attachments, source_layout, source_reference, repost_policy, expires_at, ttl_seconds, ttl_mode, parent_message_id "
    ...
    def _api_get_digestion_manager() -> Any:
        return current_app.config.get('DIGESTION_MANAGER')

    def _api_digestion_error(exc: DigestionError):
        return jsonify({
            'success': False,
            'error': str(exc),
    ...
    @api.route('/digestions', methods=['GET'])
    @require_auth(Permission.READ_FILES)
    def list_digestions_api():
        """List Digestions accessible to the authenticated user."""
        manager = _api_get_digestion_manager()
        if not manager:
            return jsonify({'error': 'Digestion manager unavailable'}), 503
        try:
            include_sources = str(request.args.get('include_sources') or '').strip().lower() in {'1', 'true', 'yes'}
            items = manager.list_digestions(g.api_key_info.user_id, include_sources=include_sources)
            return jsonify({'success': True, 'digestions': items, 'count': len(items)})
        except Exception as e:
            logger.error("Digestion API list failed: %s", e, exc_info=True)
            return jsonify({'error': 'Internal server error'}), 500

    @api.route('/digestions', methods=['POST'])
    @require_auth(Permission.WRITE_FILES)
    def create_digestion_api():
        """Create a local Digestion over selected user-owned Vault files."""
        manager = _api_get_digestion_manager()
        if not manager:
            return jsonify({'error': 'Digestion manager unavailable'}), 503
        data = request.get_json(silent=True) or {}
        try:
            ...
            item = manager.create_digestion(
                ...
                chunk_size=_api_int_param(data.get('chunk_size'), default=1800, minimum=240, maximum=8000),
                chunk_overlap=_api_int_param(data.get('chunk_overlap'), default=220, minimum=0, maximum=2000),
                settings=data.get('settings') if isinstance(data.get('settings'), dict) else {},
            )
            status_code = 201
            if bool(data.get('auto_build') or data.get('build_now')):
                build_result = manager.build_digestion(item['id'], g.api_key_info.user_id, rebuild=True)
                item = manager.get_digestion(item['id'], user_id=g.api_key_info.user_id) or item
                item['build_result'] = build_result
            return jsonify({'success': True, 'digestion': item, 'digestion_id': item.get('id')}), status_code
        except DigestionError as exc:
            return _api_digestion_error(exc)
        except Exception as e:
            logger.error("Digestion API create failed: %s", e, exc_info=True)
            return jsonify({'error': 'Internal server error'}), 500
    ...
    def list_digestion_sources_api(digestion_id: str):
        """List source metadata for an accessible Digestion."""
        manager = _api_get_digestion_manager()
        if not manager:
            return jsonify({'error': 'Digestion manager unavailable'}), 503
        try:
            ...
            logger.error("Digestion API sources failed: %s", e, exc_info=True)
            return jsonify({'error': 'Internal server error'}), 500
    ...
    def build_digestion_api(digestion_id: str):
        """Synchronously build or rebuild a Digestion index."""
        manager = _api_get_digestion_manager()
        if not manager:
            return jsonify({'error': 'Digestion manager unavailable'}), 503
        data = request.get_json(silent=True) or {}
    ...
    def query_digestion_api(digestion_id: str):
        """Query a Digestion and return cited source chunks."""
        manager = _api_get_digestion_manager()
        if not manager:
            return jsonify({'error': 'Digestion manager unavailable'}), 503
        data = request.get_json(silent=True) or {}
    ...
    def digestion_visual_evidence_api(digestion_id: str):
        """List source-derived PDF visual evidence records for an accessible Digestion."""
        manager = _api_get_digestion_manager()
        if not manager:
            return jsonify({'error': 'Digestion manager unavailable'}), 503
        try:
            limit = _api_int_param(request.args.get('limit'), default=160, minimum=1, maximum=320)
            ...
            logger.error("Digestion API visual evidence failed: %s", e, exc_info=True)
            return jsonify({'error': 'Internal server error'}), 500
    ...
    def grant_digestion_access_api(digestion_id: str):
        """Grant query/manage access to another local user or agent."""
        manager = _api_get_digestion_manager()
        if not manager:
            return jsonify({'error': 'Digestion manager unavailable'}), 503
        data = request.get_json(silent=True) or {}
    ...
    def repost_channel_message_api(channel_id, message_id):
        ...
        try:
            ...
            if not eligibility.get('allowed'):
                return jsonify({'error': eligibility.get('reason') or 'Cannot repost this message'}), int(eligibility.get('status_code') or 400)
    ...
    def _api_trigger_member_sync(db_manager: Any, channel_manager: Any, p2p_manager: Any,
        ...
        if not p2p_manager or not p2p_manager.is_running():
            return
        try:
            default_post_policy = getattr(channel_manager, 'POST_POLICY_OPEN', 'open')
            default_lifecycle_days = getattr(channel_manager, 'DEFAULT_CHANNEL_LIFECYCLE_DAYS', 180)
            with db_manager.get_connection() as conn:
                row = conn.execute(
            ...
            if action == 'add':
                _api_broadcast_channel_announce(
                    db_manager=db_manager,
                    channel_manager=channel_manager,
                    p2p_manager=p2p_manager,
                    channel_id=channel_id,
    ...
    def _api_to_bool(value: Any, default: bool = False) -> bool:
        if isinstance(value, bool):
            return value
        if value is None:
            return default
    ...
    def _api_build_channel_announce_payload(
        db_manager: Any,
        channel_manager: Any,
        p2p_manager: Any,
        channel_id: str,
    ) -> Optional[dict[str, Any]]:
        if not (db_manager and channel_manager and p2p_manager):
            return None
        try:
            default_post_policy = getattr(channel_manager, 'POST_POLICY_OPEN', 'open')
            default_lifecycle_days = getattr(channel_manager, 'DEFAULT_CHANNEL_LIFECYCLE_DAYS', 180)
            ...
            privacy_mode = str(_row_value('privacy_mode', 'open') or 'open').strip().lower()
            ...
            return {
                'channel_id': channel_id,
                'name': _row_value('name', '') or '',
                'channel_type': _row_value('channel_type', 'public') or 'public',
                'description': _row_value('description', '') or '',
                'privacy_mode': privacy_mode,
                'post_policy': str(_row_value('post_policy', default_post_policy) or default_post_policy).strip().lower(),
                'allow_member_replies': bool(_row_value('allow_member_replies', 1)),
                'allowed_poster_user_ids': channel_manager.get_channel_allowed_poster_ids(channel_id),
                'last_activity_at': _row_value('last_activity_at'),
                'lifecycle_ttl_days': _row_value('lifecycle_ttl_days', default_lifecycle_days),
                'lifecycle_preserved': bool(_row_value('lifecycle_preserved', 0)),
                'lifecycle_archived_at': _row_value('lifecycle_archived_at'),
                'lifecycle_archive_reason': _row_value('lifecycle_archive_reason'),
                'created_by_user_id': _row_value('created_by'),
                'member_peer_ids': member_peer_ids,
                'initial_members_by_peer': members_by_peer,
            }
        except Exception as e:
            logger.warning(f"Failed to build channel announce payload for {channel_id}: {e}")
            return None

    def _api_broadcast_channel_announce(
        db_manager: Any,
        channel_manager: Any,
        p2p_manager: Any,
        channel_id: str,
    ) -> bool:
        if not (p2p_manager and p2p_manager.is_running()):
            return False
        payload = _api_build_channel_announce_payload(
            db_manager=db_manager,
            channel_manager=channel_manager,
            p2p_manager=p2p_manager,
            channel_id=channel_id,
        ...
        if not payload:
            return False
        p2p_manager.broadcast_channel_announce(**payload)
        return True
    ...
    def create_channel():
        ...
        try:
            ...
            allow_member_replies = _api_to_bool(data.get('allow_member_replies'), default=True)
    ...
    def get_channel_members_api(channel_id):
        ...
        db_manager, _, _, _, channel_manager, _, _, _, _, _, _ = get_app_components(current_app)
        if not channel_manager:
            return jsonify({'error': 'Channels not available'}), 503
        try:
            access = channel_manager.get_channel_access_decision(
            ...
            policy = channel_manager.get_channel_posting_state(
                channel_id,
                g.api_key_info.user_id,
                allow_admin=bool(owner_id and owner_id == g.api_key_info.user_id),
            )
            return jsonify({
                'members': members,
                'count': len(members),
    ...
    @api.route('/channels/<channel_id>/post-policy', methods=['PATCH', 'PUT'])
    @require_auth(Permission.WRITE_FEED)
    def update_channel_post_policy_api(channel_id):
        """Update top-level posting policy for a local-origin channel."""
        db_manager, _, _, _, channel_manager, _, _, _, _, _, p2p_manager = _get_app_components_any(current_app)
        try:
            data = request.get_json() or {}
            ...
            logger.error(f"Update channel post policy failed: {e}", exc_info=True)
            return jsonify({'error': 'Internal server error'}), 500

    @api.route('/channels/<channel_id>/posters', methods=['POST'])
    @require_auth(Permission.WRITE_FEED)
    def grant_channel_poster_api(channel_id):
        """Grant explicit top-level posting access to a member."""
        db_manager, _, _, _, channel_manager, _, _, _, _, _, p2p_manager = _get_app_components_any(current_app)
        try:
            data = request.get_json() or {}
            ...
            result = channel_manager.grant_channel_post_permission(
                channel_id=channel_id,
                target_user_id=target_user_id,
                requester_id=g.api_key_info.user_id,
                allow_admin=allow_admin,
                local_peer_id=local_peer_id,
            )
            if not result:
            ...
            logger.error(f"Grant channel poster failed: {e}", exc_info=True)
            return jsonify({'error': 'Internal server error'}), 500

    @api.route('/channels/<channel_id>/posters/<user_id>', methods=['DELETE'])
    @require_auth(Permission.WRITE_FEED)
    def revoke_channel_poster_api(channel_id, user_id):
        """Revoke explicit top-level posting access from a member."""
        db_manager, _, _, _, channel_manager, _, _, _, _, _, p2p_manager = _get_app_components_any(current_app)
        try:
            local_peer_id = p2p_manager.get_peer_id() if p2p_manager else None
            owner_id = db_manager.get_instance_owner_user_id()
            ...
            logger.error(f"Revoke channel poster failed: {e}", exc_info=True)
            return jsonify({'error': 'Internal server error'}), 500
    ...
    # ------------------------------------------------------------------ #
    #  Collaboration cards (input + telemetry)                           #
    # ------------------------------------------------------------------ #
    ...
    def create_collab_card_api():
        try:
            ...
            if card_type not in ('input', 'telemetry'):
                return jsonify({'error': 'Invalid card_type', 'allowed': ['input', 'telemetry']}), 400
            title = str(data.get('title') or '').strip()
            if not title:
                return jsonify({'error': 'title required'}), 400
            actor_id = g.api_key_info.user_id
            card_id = str(data.get('id') or data.get('card_id') or '').strip() or collab_card_manager.new_ad_hoc_card_id(card_type)
            source_type = str(data.get('source_type') or 'api').strip()
            source_id = str(data.get('source_id') or card_id).strip()
            visibility = str(data.get('visibility') or 'network').strip().lower()
            if visibility not in ('network', 'local', 'public', 'private', 'custom'):
            ...
            payload = {
                ...
                'editors': data.get('editors') if isinstance(data.get('editors'), list) else [],
            ...
            if card_type == 'input':
                payload['config'] = {
                    **payload['config'],
                    'kind': data.get('kind') or payload['config'].get('kind') or 'text',
                    'options': data.get('options') if isinstance(data.get('options'), list) else payload['config'].get('options', []),
                    'required': bool(data.get('required', payload['config'].get('required', False))),
                }
            card = collab_card_manager.upsert_card(**payload)
            if not card:
                return jsonify({'error': 'Failed to create card'}), 500
            card = collab_card_manager.get_card(card_id, viewer_id=actor_id, admin_user_id=db_manager.get_instance_owner_user_id() if db_manager else None)
    ...
    def respond_collab_card_api(card_id):
        try:
            ...
            if 'value' not in data and 'comment' not in data:
                return jsonify({'error': 'value or comment required'}), 400
            admin_id = db_manager.get_instance_owner_user_id() if db_manager else None
            ...
            logger.error(f"Respond to collaboration card failed: {e}", exc_info=True)
            return jsonify({'error': 'Internal server error'}), 500

    @api.route('/collab-cards/<card_id>/telemetry', methods=['PATCH', 'POST'])
    @require_auth(Permission.WRITE_FEED, allow_session=True)
    def update_collab_card_telemetry_api(card_id):
        try:
            ...
            logger.error(f"Update collaboration card telemetry failed: {e}", exc_info=True)
            return jsonify({'error': 'Internal server error'}), 500

    @api.route('/collab-cards/<card_id>/status', methods=['PATCH', 'POST'])
    @require_auth(Permission.WRITE_FEED, allow_session=True)
    def update_collab_card_status_api(card_id):
        try:
            ...
            logger.error(f"Update collaboration card status failed: {e}", exc_info=True)
            return jsonify({'error': 'Internal server error'}), 500
