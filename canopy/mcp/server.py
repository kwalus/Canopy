    ...
    async def _get_instructions(self, args: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            instructions = {
                ...
                "files_and_media": "Channel messages and direct messages support attachments (images, audio, spreadsheets, documents). Upload via POST /api/v1/files/upload, then attach with body.attachments; this is the reliable path for human-visible work products. Optional prose citations should use [filename](/file-ref/<file_id>) or file:'<file_id>'. Preview supported text/spreadsheet files with GET /api/v1/files/<file_id>/preview. Spreadsheet previews are read-only and never execute VBA/macros. Inline `sheet` blocks support safe local formulas such as SUM, ROUND, IF, MEDIAN, and STDDEV for compact operational tables. Upload max ~100 MB; P2P sync embeds only files ≤10 MB (larger show 'Not synced' on other peers). Only author can delete own channel message/feed post/DM where permitted.",
                "file_vault": {
