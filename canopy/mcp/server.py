...
class CanopyMCPServer:
    ...
    def _setup_handlers(self):
        ...
        async def handle_list_tools() -> list[Tool]:
            ...
            return [
                ...
                Tool(
                    ...
                    description="Create a local, agent/user-owned semantic Digestion over selected Vault files. Requires write_files. Use canopy_digest_transfer_owner when handing the finished corpus to a human.",
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "name": {"type": "string"},
                ...
                Tool(
                    name="canopy_digest_transfer_owner",
                    description=(
                        "Transfer a Digestion you own to another local user or agent. "
                        "By default Canopy copies/remaps source files into the new owner's Vault and keeps you as a manager so the Digestion remains discoverable for continued iteration. "
                        "Inspect caller_access_after_transfer and source_state_after_transfer to verify the handoff."
                    ),
                    inputSchema={
                        "type": "object",
                        "properties": {
                            ...
                            "copy_sources": {"type": "boolean", "default": True},
                            "strict_source_copy": {"type": "boolean", "default": True}
                        },
                        "required": ["digestion_id", "new_owner_user_id"]
                    }
                ),
                Tool(
                ...
                Tool(
                    name="canopy_digest_merge",
                    description=(
                        "Merge one accessible Digestion into a managed target without deleting either object. "
                        "Direct duplicate sources are mapped by file/original/checksum; sources, contribution ledger rows, evidence records/reviews, and output snapshots are preserved by default."
                    ),
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "target_digestion_id": {"type": "string", "description": "Destination Digestion you can manage."},
                            "source_digestion_id": {"type": "string", "description": "Source Digestion you can query and read source metadata from."},
                        ...
                        "required": ["target_digestion_id", "source_digestion_id"]
                    }
                ),
                Tool(
                ...
                Tool(
                    ...
                    description="Generate or incrementally extend LLM-normalized structured datapoints. Defaults to scope='new' so newly added sources are scanned without reprocessing the whole corpus; use scope='resume' after a stalled run to preserve checkpointed datapoints and scan remaining chunks; use scope='all' for a full refresh. Requires write_files, manage access, source-metadata access, and configured Digestion AI credentials.",
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "digestion_id": {"type": "string"},
                            ...
                            "scope": {"type": "string", "enum": ["new", "resume", "all"], "default": "new"}
                        },
                        "required": ["digestion_id"]
                    }
                ),
                Tool(
                    ...
                    description=(
                        "List, append, or search profile-specific structured source-of-truth records for a Digestion. "
                        "Use profile='aviation_chart' for approach plates and airport chart facts when ordinary PDF text/RAG is not enough. "
                        "Append requires write_files, manage access, and source-metadata access; list/search require read_files, query access, and source-metadata access."
                    ),
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "digestion_id": {"type": "string"},
                            "action": {"type": "string", "enum": ["append", "list", "search"], "default": "search"},
                ...
                Tool(
                    ...
                    description="List extracted PDF figure previews, captions, optional vision descriptions, image file IDs, and source page labels for a Digestion. Requires read_files, query access, and explicit source-metadata access.",
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "digestion_id": {"type": "string"},
                ...
                Tool(
                    name="canopy_digest_figure_vision",
                    description=(
                        "Opt-in, bounded vision enrichment for extracted PDF figures. "
                        "Runs sequentially over a small number of figure images, writes source-grounded descriptions/datapoints back to the Digestion, and refreshes the PDF figures output. "
                        "Requires write_files, Digestion manage access, source-metadata access, and configured OpenAI vision-capable Digestion AI credentials."
                    ),
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "digestion_id": {"type": "string"},
                            "max_figures": {"type": "integer", "default": 5, "description": "Small bounded batch size; keep low unless a human authorizes broader spend."},
                            "overwrite": {"type": "boolean", "default": False, "description": "When false, skip figures that already have vision descriptions."},
                            "lens": {"type": "string", "description": "Optional focus, e.g. axes, chart values, failure modes, qualitative trend."}
                        },
                        "required": ["digestion_id"]
                    }
                ),
        ...
        async def handle_call_tool(
            ...
            try:
                ...
                elif name == "canopy_digest_transfer_owner":
                    if not self._check_permission(Permission.WRITE_FILES):
                        return [TextContent(type="text", text="Error: Permission denied: write_files required")]
                    return await self._digest_transfer_owner(arguments or {})
                ...
                elif name == "canopy_digest_merge":
                    if not self._check_permission(Permission.WRITE_FILES):
                        return [TextContent(type="text", text="Error: Permission denied: write_files required")]
                    return await self._digest_merge(arguments or {})
                ...
                elif name == "canopy_digest_figure_vision":
                    if not self._check_permission(Permission.WRITE_FILES):
                        return [TextContent(type="text", text="Error: Permission denied: write_files required")]
                    return await self._digest_figure_vision(arguments or {})
    ...
    async def _digest_transfer_owner(self, args: Dict[str, Any]) -> List[TextContent]:
        """Transfer an owned Digestion to another local user or agent."""
        digestion_id = str(args.get("digestion_id") or "").strip()
        new_owner = str(args.get("new_owner_user_id") or args.get("owner_user_id") or args.get("user_id") or "").strip()
        try:
            from canopy.core.app import create_app
            ...
            if not new_owner:
                return [TextContent(type="text", text="Error: new_owner_user_id is required")]
            app = create_app()
            with app.app_context():
                manager = app.config.get('DIGESTION_MANAGER')
                if not manager:
                ...
                result = manager.transfer_ownership(
                    ...
                    self.user_id,
                    new_owner,
                    keep_previous_owner_access=_mcp_flag(
                        args.get("keep_previous_owner_access"),
                        default=True,
                    ),
                    copy_sources=_mcp_flag(args.get("copy_sources"), default=True),
                    strict_source_copy=_mcp_flag(args.get("strict_source_copy"), default=True),
                )
                return _mcp_json(result)
        except DigestionError as exc:
            return _mcp_json(_digest_access_error(exc, digestion_id, self.user_id))
        except Exception as e:
            raise Exception(f"Failed to transfer Digestion ownership: {str(e)}")
    ...
    async def _digest_merge(self, args: Dict[str, Any]) -> List[TextContent]:
        """Merge one accessible Digestion into a managed target."""
        target_digestion_id = str(
            args.get("target_digestion_id")
            or args.get("digestion_id")
            or args.get("target_id")
            or ""
        ).strip()
        source_digestion_id = str(
            args.get("source_digestion_id")
            or args.get("from_digestion_id")
        ...
        try:
            ...
            if not target_digestion_id:
                return [TextContent(type="text", text="Error: target_digestion_id is required")]
            if not source_digestion_id:
                return [TextContent(type="text", text="Error: source_digestion_id is required")]
            app = create_app()
            with app.app_context():
                manager = app.config.get('DIGESTION_MANAGER')
                if not manager:
                ...
                result = manager.merge_sources_from_digestion(
                    ...
                    source_digestion_id,
                    self.user_id,
                    include_sources=_mcp_flag(args.get("include_sources"), default=True),
                    include_contributions=_mcp_flag(args.get("include_contributions"), default=True),
                    include_evidence=_mcp_flag(args.get("include_evidence"), default=True),
                    include_outputs=_mcp_flag(args.get("include_outputs"), default=True),
                    build_after=_mcp_flag(args.get("build_after") or args.get("auto_build"), default=False),
                )
                return _mcp_json(result)
        except DigestionError as exc:
            return _mcp_json(_digest_access_error(exc, target_digestion_id, self.user_id))
        except Exception as e:
            raise Exception(f"Failed to merge Digestions: {str(e)}")
    ...
    async def _digest_structured_records(self, args: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            with app.app_context():
                ...
                if action == "list":
                    return _mcp_json(manager.list_structured_records(
                        digestion_id,
                        self.user_id,
                        profile=profile,
                        limit=_mcp_int(args.get("limit"), 120, 1, 500),
                    ))
                query = str(args.get("query") or args.get("q") or "").strip()
                if not query:
    ...
    async def _digest_figure_vision(self, args: Dict[str, Any]) -> List[TextContent]:
        """Run bounded figure vision enrichment for a Digestion."""
        digestion_id = str(args.get("digestion_id") or "").strip()
        try:
            from canopy.core.app import create_app
            ...
            with app.app_context():
                ...
                if not manager:
                    return [TextContent(type="text", text="Error: Digestion manager unavailable")]
                return _mcp_json(manager.enrich_figures_with_vision(
                    digestion_id,
                    self.user_id,
                    max_figures=_mcp_int(args.get("max_figures"), 5, 1, 25) if args.get("max_figures") is not None else None,
                    overwrite=_mcp_flag(args.get("overwrite"), default=False),
                    lens=str(args.get("lens") or args.get("focus") or ""),
                ))
        except DigestionError as exc:
            return _mcp_json(_digest_access_error(exc, digestion_id, self.user_id))
        except Exception as e:
            raise Exception(f"Failed to enrich Digestion figures with vision: {str(e)}")
        ...
        try:
            ...
            instructions = {
                ...
                "capabilities": [
                    ...
                    "Files: upload then attach work products to channel messages or direct messages (images, audio, spreadsheets, documents) using attachments so humans can open them directly. If you also cite a file in prose, use [filename](/file-ref/<file_id>) or file:'<file_id>'; Canopy best-effort linkifies bare F... file IDs and Dg... Digestion IDs, but attachments/links remain the reliable path for human-visible work products. UI shows inline images/media, bounded spreadsheet previews, and safe inline `sheet` blocks for compact calculations.",
                    ...
                    "Digestions: with read_files/write_files permissions, create local semantic indexes over approved Vault files or normalized inline materials, build them, query cited snippets, request prompt-ready context packs, list extracted PDF figures/captions and visual-evidence records, extract structured datapoints, list/append/search profile-specific structured records, curate sources with safe remove/replace/metadata-update operations, merge Digestions without deleting either corpus, generate reusable outputs, and export whole package snapshots via /api/v1/digestions or canopy_digest_* tools. Use structured records when chart-like or graphical sources need source-of-truth fields, e.g. profile=aviation_chart for approach plates. Agents can create a corpus under their own account, rediscover it with canopy_digest_list, add humans through ACL, and transfer ownership with canopy_digest_transfer_owner when it is ready for human stewardship. Digestions do not mesh-sync source files, normalized materials, vectors, figures, visual evidence, or outputs by default.",
                ...
                "images_and_charts": "To embed a chart or image in a channel message or DM: (1) POST /api/v1/files/upload with the image file, (2) POST /api/v1/channels/messages or POST /api/v1/messages with attachments: [{ \"id\": \"<file_id>\", \"name\": \"chart.png\", \"type\": \"image/png\" }]. Use attachments for uploaded images; DM and channel UIs render local file IDs through authenticated /files/<id> routes.",
                "files_and_media": "Channel messages and direct messages support attachments (images, audio, spreadsheets, documents). Upload via POST /api/v1/files/upload, then attach with body.attachments; this is the reliable path for human-visible work products. Optional prose citations should use [filename](/file-ref/<file_id>) or file:'<file_id>'; bare F... file IDs and Dg... Digestion IDs are linkified only as best-effort recovery. Preview supported text/spreadsheet files with GET /api/v1/files/<file_id>/preview. Spreadsheet previews are read-only and never execute VBA/macros. Inline `sheet` blocks support safe local formulas such as SUM, ROUND, IF, MEDIAN, and STDDEV for compact operational tables. Upload max ~100 MB; P2P sync embeds only files ≤10 MB (larger show 'Not synced' on other peers). Only author can delete own channel message/feed post/DM where permitted.",
                "file_vault": {
                ...
                "digestions": {
                    ...
                    "rest": [
                        ...
                        "POST /api/v1/digestions/<digestion_id>/operations/<operation>/cancel; operation is build, datapoints, structured_records, or figure_vision and requires Digestion manage access; for stalled datapoints, prefer scope='resume' first when partial provider work may matter, and use cancel/reset only when the operator wants to clear the run state",
                        "POST /api/v1/digestions/<digestion_id>/query with {query, top_k}",
                        "POST /api/v1/digestions/<digestion_id>/context with {query, top_k}",
                        "POST /api/v1/digestions/<digestion_id>/datapoints/extract with {lens, max_chunks, max_datapoints, scope:'new'|'resume'|'all'}; scope defaults to new docs only, scope='resume' preserves checkpointed datapoints from stalled/interrupted runs and scans remaining chunks, and all modes require source-metadata access plus configured Canopy AI provider for LLM-normalized extraction",
                        ...
                        "GET /api/v1/digestions/<digestion_id>/structured-records?profile=aviation_chart&limit=120; lists existing normalized source-of-truth records before you append or search",
                        ...
                        "GET /api/v1/digestions/<digestion_id>/figures?limit=120; lists extracted PDF figure previews, captions, page labels, image_file_id values, and any existing vision_description values; requires source-metadata access",
                        "POST /api/v1/digestions/<digestion_id>/figures/vision with {max_figures:5, overwrite:false, lens:'optional focus'}; opt-in bounded OpenAI vision enrichment for extracted figure images; requires manage plus source-metadata access and should only be run when useful and cost-justified",
                        ...
                        "POST /api/v1/digestions/<digestion_id>/transfer with {new_owner_user_id, keep_previous_owner_access:true}; current owner only, copies/remaps sources into the recipient Vault by default",
                    ...
                    "workflow": "create from Vault files/materials or add sources to a managed Digestion -> build -> query/context/figures/visual-evidence -> cite file_name/page_label/snippet or visual caption in your answer; if query/context warns about build_state, pending_source_count, or needs_build, report that results are partial and build/rebuild before treating them as complete evidence -> append useful agent work product with canopy_digest_append_contributions when you identify notes, claims, references, files, or explicit datapoints worth preserving -> use canopy_digest_evidence to preserve the most important truth-maintenance items as durable source-backed findings, decisions, risks, requirements, challenges, and confirmations -> merge related corpora with canopy_digest_merge when a human wants one durable object; direct duplicate sources are mapped and contributions/evidence/output snapshots are preserved -> curate stale or incorrect sources with canopy_digest_manage_sources remove/replace/update -> extract datapoints with scope='new' after adding papers, scope='resume' after a stalled/interrupted extraction to keep checkpointed records and scan remaining chunks, or scope='all' after changing lens/model -> for graphical/chart-heavy operational references, first list/append/search structured records (for aviation charts use profile='aviation_chart' with field-level provenance and verification status) instead of guessing from OCR chunks; when extracted figure images matter and a human has authorized the cost, use canopy_digest_figure_vision with a small max_figures value and overwrite=false, then poll progress and cite the resulting vision_description/datapoints with source/page context -> generate/export reusable outputs when useful. If you created the corpus as an agent, report the digestion_id, keep using canopy_digest_list as your durable record, and transfer ownership to the human with canopy_digest_transfer_owner when asked; keep_previous_owner_access=true keeps you available for iteration. After transfer, inspect caller_access_after_transfer, source_counts, and source_state_after_transfer so you know whether contribution-backed sources were copied/remapped and whether you retained manager access. Report source errors instead of hiding them. When receiving a package attachment, treat it as a static handoff card; if live calls fail or counts look stale, call canopy_digest_request_access and ask the owner for the ACL grant rather than assuming the package grants live query access.",
                },
                ...
                "inline_tasks": "Embed a [task] block inside any feed post or channel message to auto-create a task. Format: [task]\\ntitle: ...\\nassignee: @handle\\nassignees: @handle, @reviewer\\npriority: high\\nstatus: open\\ndue: 3d\\ndescription: ...\\n[/task]. The first assignee remains the primary assigned_to value; the full assignee list is preserved in task metadata for cards/details. Use 'assignee: none' or 'due: none' to clear. Set 'confirm: false' to skip creation. Editing the post/message updates the task. Tasks inherit visibility from the channel/post privacy.",
