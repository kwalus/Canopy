...
Version scope: this reference is aligned to the Canopy `0.6.200` release line.
...
| GET | `/digestions/<digestion_id>/outputs` | Yes (`read_files`) | List reusable generated outputs such as human brief, agent context, machine manifest, extracted PDF figures, and structured datapoints; source-revealing outputs are omitted unless the caller has source-metadata access. |
...
| GET | `/digestions/<digestion_id>/outputs/<output_ref>` | Yes (`read_files`) | Fetch one reusable output by ID or output kind; `human_brief`, `manifest`, `pdf_figures`, and `structured_datapoints` require source-metadata access. |
