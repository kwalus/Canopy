...
Version scope: this reference is aligned to the Canopy `0.6.301` release line.
...
| POST | `/digestions/<digestion_id>/figures/vision` | Yes (`write_files`) | Opt-in bounded vision enrichment for extracted PDF figure images. Requires Digestion manage access, source-metadata access, and configured OpenAI vision-capable Digestion AI credentials. Optional body: `max_figures` (default 5, max 25), `overwrite` (default false), `lens`. The response and `/progress` include `eligible_count`, `analyzed_count`, `skipped_count`, `error_count`, and bounded per-figure skipped/error rows with source/page/reason details so failed image/model runs are diagnosable. Normal builds never run this automatically. |
