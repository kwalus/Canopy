...
| POST | `/digestions/<digestion_id>/figures/vision` | Yes (`write_files`) | Opt-in bounded vision enrichment for extracted PDF figure images. Requires Digestion manage access, source-metadata access, and configured OpenAI vision-capable Digestion AI credentials. Optional body: `max_figures` (default 5, max 25), `overwrite` (default false), `lens`. Poll `/progress` for `operations.figure_vision`; normal builds never run this automatically. |
