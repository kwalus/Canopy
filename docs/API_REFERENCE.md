...
Version scope: this reference is aligned to the Canopy `0.6.305` release line.
...
- Channel Agent Run Capsules render instantly from deterministic local heuristics. When a user has `@Canopy` credentials or an instance fallback is available, the browser may call `POST /ajax/canopy_llm/capsule_summary` with a bounded visible capsule packet to get a short JSON refinement (`title`, `overview`, `key_update`, `attention`, `source_trail`, `next_action`, optional `work_effort`, optional source-linked `workproducts`). This enrichment is cached by viewer/capsule/source hash, disables web search, uses low output-token limits, and falls back silently to the deterministic capsule when unavailable.
...
| GET | `/digestions/<digestion_id>/figures` | Yes (`read_files`) | List extracted PDF figure previews, image file IDs, page labels, captions, caption-context text, and any existing opt-in vision descriptions; requires query access plus explicit source-metadata access. |
| POST | `/digestions/<digestion_id>/figures/vision` | Yes (`write_files`) | Opt-in bounded vision enrichment for extracted PDF figure images. Requires Digestion manage access, source-metadata access, and configured OpenAI vision-capable Digestion AI credentials. Optional body: `max_figures` (default 5, max 25), `overwrite` (default false), `lens`. The response and `/progress` include `eligible_count`, `analyzed_count`, `skipped_count`, `error_count`, and bounded per-figure skipped/error rows with source/page/reason details so failed image/model runs are diagnosable. Non-data images or empty source-grounded descriptions are recorded as skipped figures instead of fatal errors. Normal builds never run this automatically. |
...
| POST | `/digestions/<target_digestion_id>/merge` | Yes (`write_files`) | Non-destructively merge another accessible Digestion into a managed target. Body: `source_digestion_id`, optional `include_sources`, `include_contributions`, `include_evidence`, `include_outputs`, and `build_after`. Direct duplicate sources are mapped by file/original/checksum; source rows, contribution ledger rows, evidence records/reviews, and output snapshots are preserved by default. |
...
| POST | `/digestions/<digestion_id>/contributions/<contribution_id>` | Yes (`write_files`) | Accept, reject, or mark a ledger contribution reviewed. Body: `action` (`accept`, `reject`, or `review`), optional `note`, and optional `build_after`. Accepting a pending row promotes it into the live Digestion corpus. |
| GET | `/digestions/<digestion_id>/evidence` | Yes (`read_files`) | List durable evidence records for an accessible Digestion. Optional `status`, `query`, `tag`, `limit`, and `metadata_only`; requires Digestion query access. |
| POST | `/digestions/<digestion_id>/evidence` | Yes (`write_files`) | Append source-backed evidence records to a managed Digestion. Body: `records` with `statement`, optional `summary`, `record_kind`, `status`, `priority`, `confidence`, `tags`, `evidence_refs`, `source_refs`, `related_ids`, and `metadata`. |
| POST | `/digestions/<digestion_id>/evidence/search` | Yes (`read_files`) | Search the Digestion truth-maintenance layer by statement, summary, tags, citations, source refs, or metadata. Body: `query`, optional `status`, `tag`, and `limit`. |
| POST | `/digestions/<digestion_id>/evidence/<evidence_id>/reviews` | Yes (`write_files`) | Append a critical review event to an evidence record. Body: `action` (`support`, `challenge`, `refine`, `supersede`, `mark_stale`, `request_source`, or `confirm`), optional `note`, `confidence`, `evidence_refs`, `status`, and `superseded_by_id`. |
| POST | `/digestions/<digestion_id>/build` | Yes (`write_files`) | Synchronously build or rebuild the local index. |
| GET | `/digestions/<digestion_id>/progress` | Yes (`read_files`) | Poll live build and structured-datapoint extraction progress. Returns per-operation status, phase, percent, counts, elapsed time, checkpoint/resume details, and bounded issue details for UI/agent progress reporting. |
...
| POST | `/digestions/<digestion_id>/datapoints/extract` | Yes (`write_files`) | Generate an LLM-normalized, source-grounded `structured_datapoints` JSON output from indexed chunks. Requires Digestion manage access and a configured Canopy AI personal or admin fallback provider; the resulting output is source-revealing and requires source-metadata access to read/export. Optional body: `lens`, `max_chunks`, `max_datapoints`, `scope` (`new` default, `resume`, or `all`). `resume` preserves checkpointed datapoints from an interrupted run and scans remaining chunks. |
| POST | `/digestions/<digestion_id>/datapoints/search` | Yes (`read_files`) | Search the extracted `structured_datapoints` output by material, method, metric, claim, tag, or evidence text. This searches the normalized datapoint snapshot, not the embedded chunk index; use `/query` for semantic RAG chunk retrieval. Requires query access and source-metadata access. |
| GET | `/digestions/<digestion_id>/outputs` | Yes (`read_files`) | List reusable generated outputs such as human brief, agent context, machine manifest, extracted PDF figures, visual evidence, and structured datapoints; source-revealing outputs are omitted unless the caller has source-metadata access. |
...
| GET | `/digestions/<digestion_id>/outputs/<output_ref>` | Yes (`read_files`) | Fetch one reusable output by ID or output kind; `human_brief`, `manifest`, `pdf_figures`, `visual_evidence`, and `structured_datapoints` require source-metadata access. |
...
- Related Digestions can be merged without deleting either object. Merge is intentionally conservative: it only treats direct file/original/checksum matches as duplicates and otherwise preserves source rows, contribution ledger rows, evidence records/reviews, and output snapshots with merge provenance.
...
- PDF builds can extract embedded figure images into owner Vault-backed preview files, bind them to source page labels/caption candidates, include that context in the indexed chunks, and expose them through `/figures`, `pdf_figures` output, whole-Digestion packages, and MCP `canopy_digest_figures`. A separate opt-in figure-vision pass (`/figures/vision` or MCP `canopy_digest_figure_vision`) can send a small bounded set of extracted figure images to an OpenAI vision-capable model to populate source-grounded descriptions, author-intent notes, qualitative observations, limitations, and visible datapoints; this is never part of ordinary build and is capped so operators control cost. If a pass cannot analyze a figure, the response reports whether the cause was missing image bytes, image size, unsupported image type, non-data image content, provider/model output, empty source-grounded description, or another bounded per-figure reason. The visual-evidence pipeline additionally captures caption-derived table, chart, diagram, and figure records even when the PDF stores visuals as vector/text content without recoverable image bytes.
- Reusable outputs let a Digestion become a durable Canopy capability: a human brief for review, an agent context artifact for tool users, a machine manifest for future automation, extracted `pdf_figures` and `visual_evidence` metadata for visual/caption review, and an optional LLM-normalized `structured_datapoints` JSON snapshot for cited extracted facts. `scope:"new"` datapoint extraction incrementally scans newly indexed sources and preserves previous datapoints; `scope:"resume"` continues a stale/interrupted extraction by preserving checkpointed records and scanning remaining chunks; `scope:"all"` refreshes the full structured snapshot. Query-only grantees can use the safer `agent_context` output; source-revealing outputs remain behind explicit source-metadata access. The package endpoints produce an attachable snapshot, but live query still requires Digestion ACL access.
...
- Structured datapoint extraction uses the Canopy AI credential chain, first the requesting user's personal provider and then the admin fallback provider. It sends bounded indexed chunk excerpts to the configured LLM provider with web search disabled, records provider/model metadata in the output, and checkpoints successful batches into the `structured_datapoints` output so provider work is not silently lost after a restart or timeout.
