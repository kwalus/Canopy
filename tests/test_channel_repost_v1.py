...
class TestChannelRepostManager(unittest.TestCase):
    ...
    def test_resolve_repost_reference_no_deck_flag_for_unknown_id_only_attachment(self) -> None:
        """Unknown/id-only files should not expose a Deck button that cannot open."""
        from canopy.core.channels import MessageType
    ...
    def test_resolve_repost_reference_no_deck_flag_for_image_only_attachment(self) -> None:
        original = self.manager.send_message(
            channel_id=self.channel.id,
            user_id='author-user',
            content='Static image only',
            attachments=[{'id': 'IMG1', 'name': 'photo.png', 'type': 'image/png', 'url': '/files/IMG1'}],
        )
        self.assertIsNotNone(original)
        repost = self.manager.create_repost(original.id, 'viewer-user', self.channel.id, 'Boost image')
        self.assertIsNotNone(repost)
