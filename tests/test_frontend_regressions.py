...
class TestFrontendRegressions(unittest.TestCase):
    ...
    def test_media_deck_adds_source_level_launcher_for_playable_posts_and_messages(self) -> None:
        ...
        self.assertIn("btn.setAttribute('data-open-media-deck', '1');", main_js)
        ...
        self.assertIn("btn.innerHTML = `<i class=\"bi bi-collection-play\"></i><span class=\"canopy-media-deck-launcher-label\">Media deck</span><span class=\"canopy-media-deck-launcher-count\">${countLabel}</span>`;", main_js)
        ...
        self.assertIn("el = resolveYouTubeMediaElement(el, { activate: true, autoplay: false }) || el;", main_js)
