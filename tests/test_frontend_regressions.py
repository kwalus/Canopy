...
class TestFrontendRegressions(unittest.TestCase):
    ...
    def test_canopy_module_runtime_scaffold_and_attachment_path_exist(self) -> None:
        ...
        self.assertIn("function displayAttachments(attachments) {", channels_template)
        self.assertIn("const isCanopyModuleAttachment = (att) => {", channels_template)
        ...
        self.assertIn("const explicitItem = manifest ? buildDeckWidgetItem(manifestNode, manifest, sourceEl) : null;", main_js)
        self.assertIn("const items = mergeExplicitDeckItem(getSourceDeckItems(sourceEl), explicitItem);", main_js)
        self.assertIn("const explicitSelectedWidget = (", main_js)
        self.assertIn("state.deckItems = mergeExplicitDeckItem(", main_js)
